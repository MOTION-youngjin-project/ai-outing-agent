...
test("실제 NextAuth 로그인·본인 목록·타인 추천 읽기전용·DELETE 차단", async ({ page }) => {
  await login(page, a);
  ...
  // 타인 추천은 읽기는 되고(공유 링크) 소유자 표시만 false — 삭제는 여전히 막힌다.
  const othersRun = await page.request.get(`/api/recommend/${runB}`);
  expect(othersRun.status()).toBe(200);
  expect((await othersRun.json()).data.isOwner).toBe(false);
...
test("비회원 같은 브라우저 새로고침·다른 브라우저 읽기전용·계획 복사", async ({ page }) => {
  // 쿠키 없는 브라우저도 링크로 보는 것은 되지만(isOwner=false) 재요청 버튼은 안 보인다.
  const beforeCookie = await page.request.get(`/api/recommend/${guestRun}`);
  expect(beforeCookie.status()).toBe(200);
  expect((await beforeCookie.json()).data.isOwner).toBe(false);
  await guest(page);
...
test("주차 API 오류와 빈 목록 구분·재시도", async ({ page }) => {
  await guest(page); const failed = true;
