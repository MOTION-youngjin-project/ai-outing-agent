...
const SYSTEM_PROMPT =
  ...
  // 2026-09-12 실측: "중구에서 아이랑"에 수성구 대구미술관이 나왔다(로컬·프로덕션 각 1회, 같은 장소).
  // 시/도만 지키라는 지시밖에 없어서 유명한 곳이 구/군을 넘어 끌려온다.
  "사용자가 구/군까지 지정하면(예: '중구에서', '수성구') 반드시 그 구/군 안에 있는 장소만 추천해라. " +
  "그 구/군에 있다고 확신할 수 없는 장소는 유명하더라도 빼고, 그 구/군 안의 다른 곳을 찾아라 — " +
  "예를 들어 중구를 요청받았는데 수성구에 있는 대구미술관을 넣으면 안 된다. " +
  "그 구/군에서 3곳을 못 채우겠으면 억지로 다른 구/군을 섞지 말고 찾은 만큼만 추천하고 " +
  "왜 적은지 message에 밝혀라.\n\n" +
  ...
  // 이름이었다(2026-09-12 실측, docs/agent-latency-and-quota.md).
  "name에는 지도에서 한 건으로 검색되는 고유 장소명(간판에 적힌 이름)만 써라 — " +
  "예: '대구미술관', '국립대구박물관', '수성못'. " +
  "'및/와/,'로 두 곳을 묶은 이름, '○○거리 일대'·'실내 체험 공간'처럼 범위나 범주를 " +
  "뭉뚱그린 이름, 괄호로 부연설명을 붙인 이름('의료선교박물관 (동산의료원 내)')은 쓰지 마라. " +
  "그런 이름은 지도 검색에 잡히지 않아서 " +
  "그 장소의 길찾기·주차·거리 정보가 전부 빠진 채로 사용자에게 보인다. " +
  "여러 곳을 묶어 제안하고 싶으면 장소를 각각 따로 추천하고, 묶음 설명은 reason이나 " +
  "suggestedRoute에 써라.";
...
const PlaceSchema = z.object({
  ...
  name: z.string().describe("지도에서 한 건으로 검색되는 고유 장소명(간판 이름). 두 곳을 묶거나 범위·범주로 뭉뚱그린 이름 금지"),
