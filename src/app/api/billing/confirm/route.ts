...
import { chargeSubscription } from "@/lib/billing/subscription";

// 카드 등록 성공 시 토스가 리다이렉트로 보내는 자리(successUrl). authKey는 1회용이라
// 여기서 바로 빌링키로 바꾸고 첫 달 승인까지 끝낸다.
export async function GET(req: NextRequest) {
  ...
  // 남의 구독을 활성화시키지 못하도록, 세션의 사용자와 같은 행인지 확인한다.
  const session = await auth();
  if (!session?.user?.id) return back("error=auth");
  const sub = await prisma.subscription.findUnique({ where: { customerKey } });
  if (!sub || sub.userId !== BigInt(session.user.id)) return back("error=invalid");

  try {
    const billingKey = await issueBillingKey(authKey, customerKey);
    const withKey = await prisma.subscription.update({ where: { id: sub.id }, data: { billingKey } });
    const ok = await chargeSubscription(withKey, session.user.email);
