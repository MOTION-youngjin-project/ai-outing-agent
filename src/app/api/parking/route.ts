...
export async function GET(req: NextRequest) {
  ...
  catch { return NextResponse.json({ error: "위도·경도에 유효한 좌표를 함께 입력해 주세요." }, { status: 400 }); }
  try {
    ...
    const origin = point ?? { latitude: destination!.latitude.toNumber(), longitude: destination!.longitude.toNumber() };
    ...
    return NextResponse.json(
      { error: "주차장 조회에 실패했습니다. 다시 시도해 주세요." },
      { status: 500 }
    );
  }
}
