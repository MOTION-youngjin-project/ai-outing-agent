...
| DB | MySQL + Prisma ORM |
| 공공데이터 | 에어코리아(대기질), 기상청(날씨), 문화포털(공연/전시), 대구시 통합주차정보, NAVER API HUB 지역 검색 |
...
`NAVER_API_HUB_CLIENT_ID`와 `NAVER_API_HUB_CLIENT_SECRET`은 네이버 클라우드 플랫폼의 NAVER API HUB에서 이용 신청 후 발급받는다.
